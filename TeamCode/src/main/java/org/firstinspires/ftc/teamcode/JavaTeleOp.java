...
@TeleOp(name = "JavaTeleOp", group = "Competition")
public class JavaTeleOp extends LinearOpMode {
    ...
    private void AprilTagLockControl() {
        ...
        // Clamp forward/back, lateral, and rotation adjustments for smooth motion
        DriverForward = (float) Range.clip(-forwardError * kDistance, -maxPower, maxPower);
        DriverRight = (float) Range.clip(lateralError * kLateral, -maxPower, maxPower);
        DriverClockwise = (float) Range.clip(rotateError * kBearing, -maxPower, maxPower);

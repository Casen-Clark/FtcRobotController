...
@TeleOp(name = "JavaAprilTag2", group = "Competition")
public class JavaAprilTag2 extends LinearOpMode {
    ...
    private double AccelFactor = 0.5;       // Smooth acceleration factor
    ...
    private void AprilTagLockControl() {
        ...
        for (AprilTagDetection tag : detections) {
            if (tag.id == LockTagID) {
                ...
                actualDistance = tag.ftcPose.range;   // forward/back
                actualLateral = tag.ftcPose.x;        // left/right
                actualBearing = tag.ftcPose.bearing;  // rotation

                // Compute errors to align in front of tag
                forwardError = DesiredForward - actualDistance;
                lateralError = DesiredLateral - actualLateral;
                rotateError = -actualBearing; // rotate to face tag front
        ...
        // --- Target powers ---
        float TargetForward   = (float) Range.clip(forwardError * kDistance, -maxPower, maxPower);
        float TargetRight     = (float) Range.clip(lateralError * kLateral, -maxPower, maxPower);
        float TargetClockwise = (float) Range.clip(rotateError * kBearing, -maxPower, maxPower);
        ...
        if (IsAprilTagDetected) {
            telemetry.addData("Desired Distance", "%.2f", DesiredForward);
            telemetry.addData("Actual Distance", "%.2f", actualDistance);
            telemetry.addData("Distance Error", "%.2f", forwardError);

            telemetry.addData("Desired Lateral", "%.2f", DesiredLateral);
            telemetry.addData("Actual Lateral", "%.2f", actualLateral);
            telemetry.addData("Lateral Error", "%.2f", lateralError);

            telemetry.addData("Desired Rotate", "%.2f", DesiredRotate);
            telemetry.addData("Actual Bearing", "%.2f", actualBearing);
            telemetry.addData("Bearing Error", "%.2f", rotateError);
        }

        telemetry.addData("DriverForward", "%.2f", DriverForward);
        telemetry.addData("DriverRight", "%.2f", DriverRight);
        telemetry.addData("DriverClockwise", "%.2f", DriverClockwise);
    }
